...
Canonical Identity and Production Control — R2

The Academy uses a dual canonical identity standard for every AI Agent, Trooper, and Service Member:

- Service Member master ID: `ATA-<CALLSIGN>-000`
- Callsign ID: `ATA-SM-<CALLSIGN>-001`
- Exact callsign: `@<CALLSIGN>`
- Legacy sequential IDs (`ATA-SM-001` through `ATA-SM-066`) are migration aliases only and must not be used as new primary keys.

Identity is immutable. Roles, prompts, permissions, qualifications, tools, readiness, duty assignments, workflow assignments, telemetry, audit records, evidence, and verification are versioned dependent records. A role change must never create a second identity.
...
Fuzzy identity matching is prohibited for controlled execution. In particular, `@VICTOR` and `@TROOPER_VICTOR` are separate canonical entities. Historical role changes for identities such as `@CINDY` and `@MAPE` must be preserved as versioned duty assignments rather than competing identities.

Production identity registry

The institutional workforce baseline contains 66 controlled identities. The platform's agent registry must be seeded from the canonical personnel source rather than operating as an empty registry in production-like environments. Each registry record should carry, at minimum:
...
The database must enforce uniqueness for `service_member_id`, `callsign_id`, and exact callsign. Workflow, task, telemetry, audit, evidence, incident, release, approval, and verification records must reference the canonical `service_member_id`.

Production-readiness equation

`PRODUCTION_VERIFIED = verified identity + governed source access + enforced permissions + controlled workflow + real telemetry + audit trail + evidence + independent verification`

A role definition, prompt, dashboard, workflow, agent card, deployed component, or successful demo is not by itself proof of production readiness. Promotion to `PRODUCTION_VERIFIED` is environment- and version-specific and requires evidence for all mandatory controls.

Required production controls:

- Verified canonical identity and deterministic alias resolution
- Governed source access with provenance, freshness, and source-record identifiers
- Enforced RBAC/ABAC and mission-bound authorization with deny-by-default behavior
- Controlled workflow state transitions, approvals, retries, idempotency, handoffs, and kill switches
- Real telemetry linked to `service_member_id` and `task_id`
- Append-only audit records containing actor, action, state change, timestamp, and result
- Immutable evidence references and hashes
- Independent verification with reproducible criteria and recorded disposition

Repository implementation rule

The canonical identity contract is part of the production platform, not a parallel spreadsheet-only convention. Database schema, API contracts, seeders, frontend agent registry, workflow payloads, audit middleware, telemetry, tests, CI, governance documentation, and release evidence must all use the same identifiers and role-versioning rules.

R2 migration rule

Existing sequential IDs remain traceable aliases for migration and audit continuity. They must not be silently discarded or reused. Role history must also be retained. Any migration must preserve source lineage, prior identifiers, version history, and evidence hashes.
...
curl http://localhost:8000/v1/service-members # canonical identity registry (empty until a governed roster is imported)
...
Or visit the console pages directly: `/levels`, `/labs`, `/certifications`, `/learners`, `/glossary`, `/knowledge-base`, `/agents`, `/service-members`, `/incidents`, `/releases`, `/governance`, `/audit-log`, `/users` (admin only) — under `http://localhost:8080` (the gateway; use this one). Full endpoint reference: `10_DOCS/api/API_REFERENCE.md`.
...
R2.1 Repository Control Update

This README was updated from the preserved institutional v2.0.0 source. The exact original uploaded README is retained in `00_SOURCE/README_v2.0.0_ORIGINAL.md` for provenance and rollback. Version 2.1.0 adds the canonical identity and production-control contract without removing the original curriculum, architecture, governance, security, testing, operations, roadmap, or institutional-production architecture content.
